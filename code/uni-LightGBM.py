...
sub_df   = pd.read_csv(DATA_DIR / 'ch2026_submission_sample.csv')
...
## Daytime Feature Extractors (lifelog_date 기준) 각 raw 데이터로부터 subject×date 수준 aggregate 피처 추출
...
## Sleep-Date Feature Extractors (sleep_date 새벽 00–09시) 수면 중 wearable 신호가 가장 강력한 feature. 이 cell이 sleep_date 새벽 시간대의 wHr, wPedo, mActivity, mScreen, wLight 집계 피처를 추출
...
## Feature Engineering — Time / Lag-Rolling / Subject z-score / Target Encoding
# Time features
...
## Multi-Seed LightGBM Training 8 seeds × 5 folds × 7 targets. 시드별 예측 평균이 final prediction
lgb_params_base = {
    'objective':         'binary',
...
## Submission 생성
# OOF / test_preds 저장 (선택)
pd.DataFrame(oof_preds, columns=[f'oof_{t}' for t in TARGETS]).to_csv('oof_v9.csv', index=False)
pd.DataFrame(test_preds, columns=TARGETS).to_csv('test_preds_v9.csv', index=False)
