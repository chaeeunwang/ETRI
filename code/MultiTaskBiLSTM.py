'''
<모델>
BiLSTM 출력에 attention pooling, 마지막 시점 hidden state, static feature MLP, subject embedding을 결합해 멀티태스크 분류
...
<검증>
subject별 날짜 순서를 유지한 time-aware 5-fold CV를 사용해 leakage를 줄였고, fold별 예측을 평균내어 최종 submission을 생성
'''
import os
import gc
import math
...
TEST_CSV  = os.path.join(BASE_DIR, "ch2026_submission_sample.csv")
...
SEQ_FEATURE_COLS = [c for c in FEATURE_COLS if c not in ["subject_id", "lifelog_date"]]
SEQ_FEATURE_COLS = [c for c in SEQ_FEATURE_COLS if pd.api.types.is_numeric_dtype(train_df[c])]

print("raw num seq features:", len(SEQ_FEATURE_COLS))
...
for fold in range(N_FOLDS):
    ...
    selected_cols = select_stable_features(
        ...
        max_features=MAX_SEQ_FEATURES,
        missing_th=0.95
    )
    print(f"[fold {fold}] selected seq features: {len(selected_cols)}")
    ...
    tr_ds = SleepSequenceDataset(
        tr_part, subject_daily_dict, selected_cols,
        lookback=LOOKBACK, targets=TARGETS, is_test=False
    )
    va_ds = SleepSequenceDataset(
        va_part, subject_daily_dict, selected_cols,
        lookback=LOOKBACK, targets=TARGETS, is_test=False
    )
    te_ds = SleepSequenceDataset(
        test_df, subject_daily_dict, selected_cols,
        lookback=LOOKBACK, targets=TARGETS, is_test=True
    )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(f"Fold {fold} | Epoch {epoch:02d} | train {tr_loss:.5f} | valid {va_loss:.5f} | logloss {va_score:.5f}")
